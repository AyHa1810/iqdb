...
Launching IQDB server locally:
```bash
iqdb 0.0.0.0 5588 iqdb.sqlite
```

Launching a Docker instance of IQDB server
```bash
...
Testing and accessing the server
```bash
...
To retrieve an image, make GET request to `/images/:id`, where `:id` is the
ID number of the image.

```bash
curl 'http://localhost:5588/images/1234'
```
